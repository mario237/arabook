...
import com.mariodev.novelapp.adapters.ChapterCommentAdapter;
...
public class ChapterCommentReplyActivity extends AppCompatActivity implements ChapterCommentAdapter.OnDeleteClickListener{
    SharedPref sharedPref;

    String storyId, chapterId, userId, commentId, userReplyId, userCommentText, userCommentDateTime, replyCountText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        replyRef = FirebaseDatabase.getInstance().getReference("Novels")
                .child("Chapters").child(storyId).child(chapterId).child("comments").child(commentId);
        ...
        allRepliesRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    chapterCommentAdapter = new ChapterCommentAdapter(ChapterCommentReplyActivity.this,
                            commentModelList , "reply");
                    commentsRecycler.setAdapter(chapterCommentAdapter);
                    commentsRecycler.smoothScrollToPosition(commentModelList.size() - 1);
                    chapterCommentAdapter.notifyDataSetChanged();
                    chapterCommentAdapter.setOnDeleteClickListener(ChapterCommentReplyActivity.this);
    ...
    public void onDeleteClick(final int position, final List<CommentModel> modelList) {
        deletedReplyRef = FirebaseDatabase.getInstance().getReference("Novels").child("Chapters").child(storyId).child(chapterId).child("comments");


        AlertDialog.Builder builder = new AlertDialog.Builder(ChapterCommentReplyActivity.this , R.style.DialogTheme);
