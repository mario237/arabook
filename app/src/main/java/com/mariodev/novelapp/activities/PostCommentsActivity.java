...
public class PostCommentsActivity extends AppCompatActivity implements PostCommentAdapter.OnDeleteClickListener{
        ...
        commentRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    postCommentAdapter = new PostCommentAdapter(PostCommentsActivity.this, commentModelList , "comments");
                    commentsRecycler.setAdapter(postCommentAdapter);
                    commentsRecycler.smoothScrollToPosition(commentModelList.size() - 1);
                    postCommentAdapter.notifyDataSetChanged();
                    postCommentAdapter.setOnDeleteClickListener(PostCommentsActivity.this);
    ...
    public void onDeleteClick(final int position, final List<CommentModel> modelList) {
        deletedCommentRef =FirebaseDatabase.getInstance().getReference("Posts")
                .child(postId).child("comments");

        AlertDialog.Builder builder = new AlertDialog.Builder(PostCommentsActivity.this , R.style.DialogTheme);
